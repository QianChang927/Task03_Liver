...
class ArgParser:
    ...
    def __init__(self) -> None:
        ...
        parser.add_argument('--crop_size', type=__parse_tuple, default="(64, 64, 64)", help='数据预处理中填充/Resize后的张量大小')
        parser.add_argument('--samp_size', type=__parse_tuple, default="(64, 64, 64)", help='数据预处理中随机采样后patch的张量大小')

        # Model相关设置
        parser.add_argument('--model', type=str, choices=['UNet3D', 'VNet3D', 'UNetMONAI'], help='需要调用的模型名')
        parser.add_argument('--n_channels', type=__parse_tuple, default="(64, 128, 256, 512)", help='模型中间层的通道数设置(默认为"(64, 128, 256, 512)")')
        ...
        # Trainer相关设置
        parser.add_argument('--epochs', type=int, default=600, help='训练轮次(默认为600)')
        ...
        parser.add_argument('--roi_size', type=__parse_tuple, default="(64, 64, 64)", help='验证过程中滑动窗口的大小')
